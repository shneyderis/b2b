...
export async function parseOrderText(text: string): Promise<ParsedOrder> {
  ...
  const system =
    ...
    `Підбирай wine_id з найближчою назвою. Правила матчингу: ` +
    ...
    `(напр. "kara kermen" ↔ "Кара Кермен", "shardone" ↔ "Шардоне", "artania" ↔ "Артанія"); ` +
    `(в) часткова відповідність достатня — якщо текст містить ключові слова з назви, це матч; ` +
    `(г) кількість визначай з тексту ("3", "три", "две пляшки", "x2"); якщо не вказана — 1; ` +
    `(д) якщо позицію впевнено не впізнано — пропусти. ` +
    `partner_hint — назва закладу/ТОВ/ресторана; null якщо не вказано. ` +
